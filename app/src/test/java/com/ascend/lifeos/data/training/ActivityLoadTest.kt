...
class ActivityLoadTest {
    ...
    fun `title matching is word-start, hockey keeps its proven net`() {
        ...
        // the new primary sports resolve muscles and classify sanely
        val ride = SportCatalog.byId("ride")
        assertTrue(SportCatalog.musclesFor("ride").isNotEmpty())
        assertTrue(SportCatalog.musclesFor("yoga").isNotEmpty())
        assertTrue(SportCatalog.titleMatches(ride, "Radtour Isartal"))
        assertTrue(SportCatalog.titleMatches(ride, "Rad mo 18"))
        assertTrue(!SportCatalog.titleMatches(ride, "Radiologie Termin"))
        assertTrue(!SportCatalog.titleMatches(ride, "Fahrrad reparieren"))
        val ski = SportCatalog.byId("ski")
        assertTrue(SportCatalog.titleMatches(ski, "Ski fahren Garmisch"))
        assertTrue(!SportCatalog.titleMatches(ski, "Skizze für Kunst"))
        assertEquals("Race day", ride.dayWord)
        assertEquals("Practice day", SportCatalog.byId("yoga").dayWord)
