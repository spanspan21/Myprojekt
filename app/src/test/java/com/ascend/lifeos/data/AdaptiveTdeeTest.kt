...
class AdaptiveTdeeTest {
    ...
    fun tooFewLoggedDaysReturnsNull() {
        assertNull(AdaptiveTdee.computeFrom(intakes(9), steadyWeights(), "maintain"))
    }

    @Test
    fun tooFewWeighInsReturnsNull() {
        assertNull(AdaptiveTdee.computeFrom(intakes(14), steadyWeights(n = 3), "maintain"))
    }

    @Test
    fun tooShortWeightSpanReturnsNull() {
        assertNull(AdaptiveTdee.computeFrom(intakes(14), steadyWeights(spanDays = 13), "maintain"))
    }
    ...
    fun steadyWeightMeansExpenditureEqualsIntake() {
        val r = AdaptiveTdee.computeFrom(intakes(14), steadyWeights(), "maintain")!!
        assertEquals(2500, r.expenditure)
        assertEquals(2500, r.suggestedKcal)
    ...
    fun fallingWeightMeansBurningMoreThanEating() {
        ...
        val r = AdaptiveTdee.computeFrom(intakes(14), falling, "maintain")!!
    ...
    fun weightSamplesMayArriveUnsorted() {
        ...
        val r = AdaptiveTdee.computeFrom(intakes(14), shuffled, "maintain")!!
        assertEquals(2500, r.expenditure)
    }

    // ── Goal adjustment & rounding ───────────────────────────────────

    @Test
    fun loseGoalSuggestsDeficit() {
        val r = AdaptiveTdee.computeFrom(intakes(14), steadyWeights(), "lose")!!
        assertEquals(2050, r.suggestedKcal) // 2500 × 0.82, rounded to tens
    }

    @Test
    fun gainGoalSuggestsSurplus() {
        val r = AdaptiveTdee.computeFrom(intakes(14), steadyWeights(), "gain")!!
        assertEquals(2800, r.suggestedKcal) // 2500 × 1.12, rounded to tens
    }
    ...
    fun implausiblyLowExpenditureIsClamped() {
        val r = AdaptiveTdee.computeFrom(intakes(10, kcal = 900), steadyWeights(), "maintain")!!
        assertEquals(1400, r.expenditure)
    }

    @Test
    fun confidenceNeedsRichData() {
        val thin = AdaptiveTdee.computeFrom(intakes(14), steadyWeights(), "maintain")!!
        assertEquals("low", thin.confidence)
        val rich = AdaptiveTdee.computeFrom(intakes(18), steadyWeights(n = 8), "maintain")!!
